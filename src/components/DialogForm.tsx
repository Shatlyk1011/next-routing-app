...
const Dialog: FC<Props> = ({setSessionForm}) => {
  ...
  return (
    <dialog
      ref={dialogRef}
      className="min-w-screen [dialog:open]:flex min-h-screen items-center justify-center bg-neutral-100/25"
    >
      {/* wrapper */}
      <div className="relative flex w-xl flex-col items-center justify-center gap-4 rounded-2xl bg-white p-8 max-sm:h-svh max-sm:justify-start max-sm:pt-8">
        <header className="flex w-full justify-between">
          <button
            title="Закрыть окно"
            className="hover:bg-neutral-2 absolute right-8 top-6 rounded-full p-1 transition"
            type="button"
            ref={closeBtnRef}
          >
            <X className="h-5 w-5" />
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 w-full">
          <input onChange={onInputChange} value={form.name} name="name" className="py-2 border-b border-gray-300 hover:border-gray-400 focus:border-gray-400 outline-none" type="text" placeholder="Имя" />
          <input onChange={onInputChange} value={form.email} name="email" className="py-2 border-b border-gray-300 hover:border-gray-400 focus:border-gray-400 outline-none" type="text" placeholder="Email" />
          <FileInput handleInput={handleInput} selectedFiles={form?.files}/>

          <button className="mt-4 w-full bg-neutral-600 hover:bg-neutral-700 text-white rounded-full py-3 text-lg font-medium">
            Сохранить
          </button>
        </form>
      </div>
